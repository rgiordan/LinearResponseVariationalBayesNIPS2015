...
include("regression_poisson_componentwise_lib.jl")
include("regression_poisson_loglik_lib.jl")
...
export_symbols = [:df_json, :n, :x_mean, :true_mu, :true_epsilon,
	               :sim_id, :mu_lrvb_var, :mu_mfvb_var, :z_lrvb_var, :z_mfvb_var,
	               :tau_lrvb_var, :tau_mfvb_var, :log_tau_lrvb_var, :log_tau_mfvb_var,
...
const data_path = "/home/rgiordan/Documents/git_repos/variational_bayes/poisson_glmm/data"
...
for sim_id = sim_offset:(sim_offset + n_sims - 1)
	...
	# tau = 9 is about where the correspondence between MFVB and MCMC breaks down with n = 500.
	true_epsilon = rand_bound(0.3, 1.0)

	true_z = [ rand(Distributions.Normal(true_mu * x_vec[i], true_epsilon)) for i=1:n ];
	y_vec = [ rand(Distributions.Poisson(exp(z_i))) for z_i in true_z ];

	if false
		PyPlot.close()
		PyPlot.plot(x_vec, y_vec, "k.")
	end
	...
	##################
	# Define the full log likelihood:
	...
	if calculate_z_var
		...
		main_corr_term = vb_corr_term[main_i, z_cols_i] * (vb_corr_term[z_cols_i, z_cols_i] \ vb_corr_term[z_cols_i, main_i])
