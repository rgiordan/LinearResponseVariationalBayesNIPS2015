...
$A$ \cite{wainwright2008graphical}, so $S = -\npq^T \mpq + A(\npq)$; hence,
%
$$
  \frac{dS}{dm}
...
load("./data/new_scaling_n.Rdata")
gmm.scaling.n <- read.csv("data/new_scaling_n_results.csv", header=T)
...
ggplot(filter(gmm.scaling.n, variable %in% method.vars, p == 3)) +
  ...
  scale_x_log10(breaks=unique(gmm.scaling.n$n)[c(1, 3, 5)]) + scale_y_log10() +
  geom_abline(aes(intercept=log10(3.162278e-05), slope=1)) +
  theme(legend.position="top")

ggplot(filter(gmm.scaling.n, variable == "lrvb", p == 3)) +
  geom_point(aes(x=n, y=value, color=variable), size=3) +
  geom_line(aes(x=n, y=value, color=variable)) +
  scale_color_discrete(name="Method:", limits=method.vars,
                       labels=method.labels) +
  xlab("Number of data points (N)") + ylab("Running time (seconds)") +
  #scale_x_log10(breaks=unique(gmm.scaling.n$n)) + scale_y_log10() +
  scale_x_continuous()  +
